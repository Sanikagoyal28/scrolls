...
function LoginTeam() {
  ...
  function LogTeam(e) {
    ...
    if (login1.email && login1.password) {
      dispatch(LoginTeamThunk(data)).then((res) => {
        var y = res.payload.data.msg.replace(/\w\S*/g, function (txt) {
    ...
    <>
      <div className="register" id="regDiv">
        <div className="regFlex" id="loginHeadbox">
          ...
          />
          <p className="heading" id="registerCA">
            Login as <span id="member">Team</span>
          </p>
          <img
            className="cross"
            src={cross}
        ...
        <form className="allForm" onSubmit={LogTeam} id="loginForm">
          ...
          <input
            ...
            value={login1.email}
            onChange={(e) => setLogin1({ ...login1, email: e.target.value })}
          />
          ...
          <input
            ...
            className="regInputname inputPwd"
            placeholder="Enter password"
            value={login1.password}
            onChange={(e) => setLogin1({ ...login1, password: e.target.value })}
          />
          <p
          ...
          <button className="regContinue" type="submit">
            Continue
          </button>
        </form>
      </div>
